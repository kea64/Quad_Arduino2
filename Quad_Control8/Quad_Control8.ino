...
void processInterrupts(class MODE_REGISTER &mode, class ORIENTATION_REGISTER &orient, class TARGET_REGISTER &target){
...
void updateTarget(class TARGET_REGISTER &target){
   ...
   //--------------------Channel3-------------------//
   if (mode.CH3_ENABLE){
     target.throttle = (channel3Cycle-100) * 0.09;
     if (target.throttle <= 0.0){
       target.throttle = 0.0;
     }
     if (target.throttle >= 90.0){
       target.throttle = 90.0;
     }
   }
...
void updatePID(class MODE_REGISTER &mode, class ORIENTATION_REGISTER &orient, class TARGET_REGISTER &targetRegister){
  if (mode.CH1_ENABLE){
...
double calcPID(double target, double currPos, const double kp, const double ki, const double kd){
  //int error = 
}
