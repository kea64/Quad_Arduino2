...
float GyroX,GyroY,GyroZ,GyroTemp,biasGyroX, biasGyroY, biasGyroZ, biasAccelX, biasAccelY, biasAccelZ,biasMagHeading,xAng,yAng,cycle,pitchGyro,rollGyro,yawGyro,pitchPrediction,rollPrediction,yawPrediction,accX,accY,accZ,CMy,CMx,heading;
float timeStep = 0.02;
float pitchAccel = 0;
float rollAccel = 0;
float yawMag = 0;
...
void kalman(){
  ...
  if (yawGyro < 0 && yawMag > 270) {yawGyro += 360;}
  if (yawGyro > 270 && yawMag < 90) {yawGyro -= 360;}
  if (yawPrediction < 0 && yawMag > 270) {yawPrediction += 360;}
  if (yawPrediction > 270 && yawMag < 90) {yawPrediction -= 360;}
...
void calcTiltHeading(){
  ...
  CMx = mX * cos(radians(pitchPrediction)) + mZ * sin(radians(pitchPrediction));
  CMy = mX * sin(radians(rollPrediction)) * sin(radians(pitchPrediction)) + mY * cos(radians(rollPrediction)) - mZ * sin(radians(rollPrediction)) * cos(radians(pitchPrediction));
